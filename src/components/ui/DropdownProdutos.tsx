...
export default function DropdownProdutos() {
    ...
    const submenuData: { [key: string]: { id: number; nome: string }[] } = {
        "Rações e Alimentação": [
            { id: 1, nome: "Ração Seca" }, { id: 2, nome: "Ração Umida" },
            { id: 3, nome: "Ração Natural" }, { id: 4, nome: "Ração Medicamentosa" },
        ],
        "Brinquedos": [
            { id: 1, nome: "Pelúcias" }, { id: 2, nome: "Bolas" }, { id: 3, nome: "Cordas" },
        ],
        "Higiene e Beleza": [
            { id: 1, nome: "Tapetes" }, { id: 2, nome: "Fraldas" }, { id: 3, nome: "Coletor de fezes" },
            { id: 4, nome: "Alicates de unha" }, { id: 5, nome: "Acessórios para banho" },
        ],
        "Acessórios": [
            { id: 1, nome: "Comedouros" }, { id: 2, nome: "Bebedouros" },
            { id: 3, nome: "Porta rações" }, { id: 4, nome: "Mamadeiras" },
        ],
        "Medicamentos": [
            { id: 1, nome: "Vermífugos" }, { id: 2, nome: "Anti-inflamatórios" },
            { id: 3, nome: "Antibitóticos" }, { id: 4, nome: "Antipulgas e carrapatos" },
        ]
    };

    // --- Funções de navegação e controle do menu ---
    const handleCategoryClick = (categoria: string) => {
        const rota = categoria === "Ver Todos os Produtos"
            ? "/produtos"
    ...
    const handleMouseLeaveMenu = () => {
        closeTimeout.current = setTimeout(() => {
            setIsOpen(false);
            setSubmenuOpen(null);
        }, 150); // Aumentei um pouco o delay para melhor experiência
    };
